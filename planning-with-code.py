...
# Define the rules for each column
color_groups = {
...
# Fill colors column with NEW rule for categories with fewer than 2 items
for item in collection_data:
    # Check if category has fewer than 2 items for this gender
    category_count = sum(1 for i in collection_data
                        if i['category'] == item['category'] and i['gender'] == item['gender'])

    if category_count < 2:
        # Use only groups 1 and 2 for categories with fewer than 2 items
...
# Fill silhouettes column with special rules
...
for item in collection_data:
    if item['silhouettes'] == '':
        ...
        # NEW RULE: No shirt dress for men
...
for gender in ['Feminine', 'Masculine']:
    # Get details that are actually eligible for this gender (considering category constraints)
    ...
    if not check_all_options_used(collection_data, 'details', eligible_details_for_gender, gender):
        # Find missing details for this gender
        ...
        if missing_details:
            ...
            if changeable_items:
                # Change one item to use a missing detail
                item_to_change = random.choice(changeable_items)

                # Make sure the new detail is eligible for this item's category
...
def display_table(data):
    # Find maximum lengths for each column for formatting
...
# NEW: Check all surface options are used for both genders
...
# NEW: Check all detail options are used for both genders
...
# Add this line at the very end of your script to export the data:
